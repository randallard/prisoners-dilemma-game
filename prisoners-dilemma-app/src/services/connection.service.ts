// connection.service.ts - Refactored for immutability
...
export class ConnectionService {
  ...
  public getConnectionById(connectionId: string): Result<ConnectionData, ConnectionError> {
    ...
    try {
      ...
      return Result.success({...connection}); // Return a copy, not the original
    } catch (error) {
  ...
  public getConnectionsByStatus(status: ConnectionStatus): Result<ConnectionData[], ConnectionError> {
    ...
    try {
      ...
      const connections = connectionsResult.getValue();
      // filter() already creates a new array, but let's map to ensure each object is also a copy
      const filteredConnections = connections
        .filter(conn => conn.status === status)
        .map(conn => ({...conn})); // Create a new object for each connection
  ...
  public acceptConnection(connectionId: string): Result<boolean, ConnectionError> {
    ...
    try {
      ...
      // Create a new array with the updated connection
      ...
      const saveResult = this.saveConnections(updatedConnections);
      if (saveResult.isFailure()) {
        return Result.failure(saveResult.getError());
      }
  ...
  private saveConnection(connection: ConnectionData): Result<boolean, ConnectionError> {
    ...
    try {
      ...
      const connections = connectionsResult.getValue();
      // Create a new array with the new connection added
      const updatedConnections = [...connections, connection];
      
      return this.saveConnections(updatedConnections);
    } catch (error) {
  ...
  private loadConnections(): Result<ConnectionData[], ConnectionError> {
    try {
      ...
      // Validate each connection object - create a new array with only valid connections
      const validConnections = parsedData
        .filter(item => this.isValidConnectionData(item))
        .map(item => ({...item})); // Create a copy of each item
